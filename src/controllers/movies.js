...
//     const data = req.body
//     movieModel.listMovies(data, (results) => {
//         return res.json({
//             success: true,
...
exports.createMovies = (req, res) => {
    const data = req.body
    movieModel.createMovies(data, (results) => {
        if (results.affectedRows > 0) { // ngecek berhasil nda create movie nya?
            movieModel.getMovieById(results.insertId, (finalResult) => { // ngambil movie yang baru aja kamu create
                if (finalResult.length > 0) { // ngecek berhasil nda ngambilnya?
                    return res.json({ // return ke user kalo berhasil
                        success: true,
                        message: 'Details of Movie',
                        results: finalResult[0]
                    })
                }
                return res.status(400).json({ // return ke user kalo gagal
                    success: false,
